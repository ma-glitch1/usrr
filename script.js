...
const FIRESTORE_REST_BASE =
  "https://firestore.googleapis.com/v1/projects/jhjh-175e2/databases/(default)/documents";
const FIRESTORE_API_KEY = "AIzaSyDKnlA4fe5DsFSB7Yn7yT8emk8uy2GFmMY";
...
function usePreviewBunnyProxy(img) {
  ...
  // The Bunny CDN URLs are public and work directly in local previews.
  // Rewriting them to /__bunny_proxy made every image return 404 when the
  // project was served by a standard static server.
  img.dataset.originalCdnSrc = src;
}
...
function parseFirestoreDoc(doc) {
  ...
  return { firestoreId: (doc.name || "").split("/").pop(), ...data };
}

async function fetchFirestoreCollection(collectionName) {
  const url = `${FIRESTORE_REST_BASE}/${collectionName}?key=${FIRESTORE_API_KEY}`;
  const response = await fetch(url, { cache: "no-store" });
  if (!response.ok) throw new Error(`Firestore REST ${collectionName}: ${response.status}`);
  const data = await response.json();
  return (data.documents || []).map(parseFirestoreDoc);
}

async function syncDataWithFirestoreRest() {
  try {
    const [newProductsRaw, newCatsRaw] = await Promise.all([
      fetchFirestoreCollection("products"),
      fetchFirestoreCollection("categories"),
    ]);
    const newProducts = stripInlineImages(newProductsRaw);
    const newCats = stripInlineImages(newCatsRaw);
    ...
    console.error("Firestore REST sync error:", error);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  syncDataWithFirestoreRest();
  setTimeout(syncDataWithFirestoreRest, 2500);
});
...
async function syncDataWithFirebase() {
  const { getDoc, doc, collection, getDocs } = window.firestore;
  const db = window.db;

  try {
    const metaDocRef = doc(db, "meta", "version");
    ...
    if (metaDoc.exists()) {
      serverVersion = metaDoc.data().updatedAt?.toMillis() || 0;
      const localVersion = parseInt(localStorage.getItem("metaVersion") || "0");
      ...
      if (oldProducts.length > 0 || oldCats.length > 0 || (localVersion && localVersion !== '0')) {
         shouldSync = true;
      }
    }
    ...
    if (shouldSync) {
        ...
        let newProducts = productsSnap.docs.map((d) => ({
          id: d.id,
          ...d.data(),
        }));
        let newCats = catsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        ...
        newProducts = stripInlineImages(newProducts);
        localStorage.setItem("products", JSON.stringify(newProducts));
        products = newProducts;
        
        newCats = stripInlineImages(newCats);
        localStorage.setItem("categories", JSON.stringify(newCats));
...
function listenForCatalogChanges() {
  if (catalogListenerStarted || !window.db || !window.firestore) return;
  ...
  onSnapshot(
    ...
    (snapshot) => {
      const liveProducts = stripInlineImages(
  ...
  onSnapshot(
    collection(window.db, "categories"),
    (snapshot) => {
      const liveCategories = stripInlineImages(
        snapshot.docs.map((item) => ({ id: item.id, ...item.data() })),
      );
      localStorage.setItem("categories", JSON.stringify(liveCategories));
      renderCategories();
      renderSubcategories();
      renderProducts();
      refreshBunnyImages();
    },
    (error) => console.error("Categories realtime sync error:", error),
  );
}
...
function getTopLevelCategories() {
  return getCategoriesData().filter((cat) => !cat.parentId);
}
...
function getGeneratedSubcategories(categoryId) {
  ...
  categoryProducts.forEach((product) => {
    ...
    const label = words[0] || product.name || "منتجات";
    const id = `auto-${categoryId}-${label}`;
...
function getSubcategoriesForActiveCategory() {
  if (!activeCategory) return [];

  const savedSubcategories = getCategoriesData().filter(
    (cat) => cat.parentId === activeCategory,
  );
...
function productMatchesSubcategory(product, subcategory) {
  ...
  if (subcategory.parentId && !subcategory.autoKeyword) {
    return product.category === subcategory.id;
  }

  const productName = normalizeText(product.name);
  return productName.includes(normalizeText(subcategory.autoKeyword || subcategory.name));
}
  ...
  categories.forEach((cat) => {
    ...
    item.innerHTML = `
            <div class="cat-image-container ${cat.colorClass || "blue"}">
                <img src="${cat.image || IMAGE_FALLBACK_URL}" alt="${cat.name}" onerror="recoverBunnyImage(this)">
            </div>
            <span>${cat.name}</span>
        `;
...
function renderSubcategories() {
  ...
  const subcategories = getSubcategoriesForActiveCategory();
  list.innerHTML = "";
  ...
  subcategories.forEach((subcategory) => {
    ...
    item.innerHTML = `
      <img src="${subcategory.image || IMAGE_FALLBACK_URL}" alt="${subcategory.name}" onerror="recoverBunnyImage(this)">
...
function renderProducts() {
  ...
  const productsDivider = document.getElementById("products-divider");
  if (!productsList) return;

  productsList.innerHTML = "";
  if (productsDivider) {
    productsDivider.classList.toggle("active", Boolean(activeSubcategory));
  }
  ...
  let filteredProducts = products.filter(
    (product) => product.category === activeSubcategory.id,
  );

  if (activeCategory && activeSubcategory) {
    if (activeSubcategory.parentId && !activeSubcategory.autoKeyword) {
  ...
  } else if (activeCategory && getSubcategoriesForActiveCategory().length > 0) {
    productsList.innerHTML =
      '<div class="choose-subcategory-message">اختر قالب الفئة لعرض المنتجات الخاصة به.</div>';
    return;
  }

  if (filteredProducts.length === 0 && activeSubcategory && activeSubcategory.parentId) {
    filteredProducts = products.filter(
      (product) => product.category === activeSubcategory.parentId,
    );
  }
  ...
  filteredProducts.forEach((product) => {
    ...
    const btnText = inCart ? "تم التحديد" : "إضافة للسلة";
    ...
    card.innerHTML = `
            <div class="product-img-wrapper" style="position: relative;">
                ...
                <img src="${product.image || IMAGE_FALLBACK_URL}" loading="lazy" alt="${product.name}" onerror="recoverBunnyImage(this)">
            </div>
...
function renderCart() {
  ...
  if (cartItems.length === 0) {
    ...
    cartItems.forEach((item) => {
      ...
      itemEl.innerHTML = `
                <img src="${item.image || IMAGE_FALLBACK_URL}" alt="${item.name}" class="cart-item-image" onerror="recoverBunnyImage(this)">
...
function initCartModal() {
  ...
  if (submitOrderBtn) {
    submitOrderBtn.addEventListener("click", () => {
      ...
      const savedDeliveryCostValue = localStorage.getItem("deliveryCost");
      const savedDeliveryCost = Number(savedDeliveryCostValue);
      const orderDeliveryCost =
        savedDeliveryCostValue !== null &&
        savedDeliveryCostValue.trim() !== "" &&
        Number.isFinite(savedDeliveryCost) &&
        savedDeliveryCost > 0
          ? savedDeliveryCost
          : 3000;
      const orderTotal = orderSubtotal + orderDeliveryCost;
...
function initSlider() {
  ...
  // Clear previous interval if any (not strictly necessary here but good practice)
